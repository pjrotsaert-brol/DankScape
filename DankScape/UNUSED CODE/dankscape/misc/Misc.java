...
public class Misc {
    ...
    public static String getOpString(int opcode) {
        return jdk.internal.org.objectweb.asm.util.Printer.OPCODES[opcode];
    }
        ...
        for (int I = 0; I < instructions.length; ++I) {
            ...
            if (instruction == null)
                ...
                if (instruction.getOpcode() != -1) {
                    ...
                    if (value != null) {
                        System.out.println(" -> " + getOpString(instruction.getOpcode()) + value);
                    } else {
                        if (instruction instanceof MethodInsnNode) {
                            MethodInsnNode m = (MethodInsnNode) instruction;
                            System.out.println(" -> " + getOpString(instruction.getOpcode()) + "   " + m.owner + "/" + m.name + m.desc);
                        } else if (instruction instanceof FieldInsnNode) {
                            FieldInsnNode f = (FieldInsnNode) instruction;
                            System.out.println(" -> " + getOpString(instruction.getOpcode()) + "   " + f.owner + "/" + f.name + " " + f.desc);
                        } else {
                            System.out.println(" -> " + getOpString(instruction.getOpcode()));
                        }
                    }
                } else {
